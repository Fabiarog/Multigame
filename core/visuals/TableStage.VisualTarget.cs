...
public partial class TableStage
{
    ...
    private void ApplyVisualTarget(bool forward, bool detailed)
    {
        ...
        _environment.TonemapExposure = classic ? 1.05f : mexico ? .92f : madrid ? 1.08f : 1.10f;
        _environment.AmbientLightColor = new Color(classic ? "#80949e" : mexico ? "#465b80" : madrid ? "#7d5844" : "#92aba2");
        _key.LightEnergy = mexico ? .28f : classic ? 1.1f : 1.35f;
        _key.LightColor = new Color(mexico ? "#8ca9dc" : "#fff2d6");
        if (mexico) _environment.AmbientLightEnergy = .16f;
        _key.LightAngularDistance = classic && detailed ? .65f : 0;
        _key.ShadowNormalBias = classic ? .35f : 1.8f;
        _key.ShadowBias = classic ? .035f : .08f;
        _basePendantEnergy = mexico ? 1.15f : classic ? 1.55f : 1.6f;
        _pendant.LightEnergy = _basePendantEnergy;
        _pendant.OmniRange = mexico ? 6f : classic ? 6.2f : 9f;
        _pendant.LightSize = classic && detailed ? .30f : 0;
        _fillLight.Position = classic || mexico ? new Vector3(-1.8f, 2.2f, .3f) : new Vector3(0,-1.8f,.2f);
        _fillLight.LightColor = new Color(classic ? "#afc5ce" : mexico ? "#c2b29b" : "#4a3525");
        _fillLight.LightEnergy = classic ? 1.1f : mexico ? .6f : .28f;
        _fillLight.Visible = classic || mexico || forward;
        _rimLight.LightEnergy = classic ? .48f : _currentRoomTheme == "cyber_casino" ? .95f : mexico ? .32f : madrid ? .85f : .75f;
