...
- **Status Atual — ETAPAS CONCLUÍDAS E VALIDADAS (Patch 10 & Patch 11):**
  - **Separação Anatômica Modular em 9 Partes (Blender 5.2):** Todos os 8 personagens (`nina`, `bento`, `corvo`, `onca`, `iara`, `zeca`, `barao`, `dama`) foram reconstruídos com 9 nós de malha independentes (`PelvisMesh`, `BodyMesh`, `HeadMesh`, `ArmLMesh`, `ArmRMesh`, `ForearmLMesh`, `ForearmRMesh`, `HandLMesh`, `HandRMesh`). Eliminação total de torções esticadas: ombros, cotovelos e pulsos articulados livremente; pernas e sapatos repousados na poltrona sem distorcer com a respiração ou inclinação do tronco.
  - **Animação de Jogada na Mesa (`play_card`):** Novo clipe criado para todos os 8 personagens (totalizando 7 animações por personagem: `idle`, `entrance`, `truco`, `victory`, `boss_intro`, `flourish`, `play_card`). Coreografia de 48 frames Bezier com extensão do braço, snap do pulso na mesa e retorno suave à poltrona.
  - **Imersão em Primeira Pessoa (POV):** No modo POV, em vez de ocultar o ator inteiro, apenas a cabeça do jogador local é desativada. O tronco, lapelas, ombros, antebraços e mãos continuam visíveis na visão periférica. Ao lançar cartas, `TableStage` aciona a animação de estender o braço e bater a carta na mesa com som sincronizado no feltro.
  - **Corte e Rodízio do Baralho (Truco e Fodinha):** Concluído e validado. Bots embaralham, cortam e distribuem automaticamente. O jogador humano corta estritamente quando é a sua vez. Decisões de Pena no Truco tratadas autonomamente pela IA ou pelo jogador local quando destinatário.
  - **Câmera em Primeira Pessoa (POV) Elevada e com Limites de Pescoço:** Implementada e validada. Câmera a 2,35 m acima da origem do assento (olhos a 1,63 m de altura), FOV vertical de 50°. Arraste com botão direito do mouse permite olhar em até ±48° horizontal e ±12° vertical com inclinação suave do corpo e respiração orgânica. Alternância com tecla `C` e botão recenteador.
  ...
  - **Executável Exportado:** `Game Hub.exe` e `Game Hub.pck` gerados com sucesso para Windows Desktop e testados.
